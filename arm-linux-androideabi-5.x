...
# Build Configuration
./configure $PREFIX $UBERROOT --host=x86_64-linux-gnu --build=x86_64-linux-gnu --target=arm-linux-androideabi --program-transform-name='s&^&arm-linux-androideabi-&' --with-gcc-version=UBER --with-pkgversion='UBER-LINARO-5.x.x' --with-binutils-version=current --with-gold-version=current --with-gmp-version=current --with-mpfr-version=current --with-gdb-version=current --with-mpc-version=current --with-cloog-version=current --with-isl-version=current --with-host-libstdcxx='-static-libgcc -Wl,-Bstatic,-lstdc++,-Bdynamic -lm' --with-gxx-include-dir=$UBERROOT_DEST_PATH/c++ --enable-graphite=yes "--enable-eh-frame-hdr-for-static" --enable-initfini-array --enable-gold=default --enable-threads --enable-multilib --with-libexpat --enable-targets=all --with-python --with-gnu-ld --with-gnu-as --disable-werror --disable-option-checking --disable-bootstrap --disable-libsanitizer --disable-libgomp --quiet --enable-plugins;

# Build the toolchain
echo ""
echo "${bldblu}Building your UBER LINARO arm-linux-androideabi-5.x Toolchain!!!${txtrst}"
echo ""
all1=$(date +%s.%N)
script -q $DIR/out/UBER-LINARO-ANDROIDEABI-5.x.log -c "make 1>/dev/null $MAKE_FLAGS";
...
then
    ...
    echo "${bldgrn} ${txtrst}"
    echo "${bldgrn}Your UBER-LINARO 5.x.x androideabi Toolchain has completed successfully!!! ${txtrst}"
